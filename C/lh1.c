...
int main(void)
{
    ...
    /* ----------------------------------------------------- */
    // Physical:
    double  ethe, ekin, etot;   // Thermal, Kinetic, Total energies
    ...
    for (istep = 0; istep < nsteps; ++istep)
    {
        ...
        if (istep%10 == 0.)
            fprintf(fout2, "%d %lf %lf %lf %lf\n",
                        istep, t, etot, ethe, ekin);
        if (istep%100 == 0.)
            printf("istep = %d, \
                            t = %lf, \
                            etot = %lf, \
        ...
        if (istep%DUMPRES == 0.)
            for (ix = 0; ix < NX-1; ++ix)
                fprintf(fout1, "%d %lf %d %lf %lf %lf %lf %lf %lf %lf\n", 
                        istep, t, ix, fm[ix], r[ix]/r[NX-1], u[ix],
